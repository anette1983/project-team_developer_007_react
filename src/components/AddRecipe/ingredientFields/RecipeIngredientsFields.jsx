import css from "./RecipeIngredientsFields.module.css";ingredientsQuantity
...
export const RecipeIngredients = ({
    ...
    const ingredientsQuantityMeasure = ["tbs", "tsp", "kg", "g", "ml", "to taste"];
    ...
    const onInputChange = (index, value, id) => {
        ...
        ingredientList.filter((item) => item.name.toLowerCase().includes(value.toLowerCase()))
        );
    ...
    return (
        ...
        <div className={css.subtitleWrapper}>
            ...
            <button className={css.counterBtn} type="button" onClick={decrementFields}>
                <MdRemove className={css.counterBtnIcon} size="14"/>
            </button>
            <p className={css.counterValue}>{field}</p>
            <button className={css.counterBtn} type="button" onClick={incrementFields}>
                ...
                return (
                    <li className={css.ingredientItem} key={item.id}>
                        <div className={css.ingredientNameField}>
                            <div>
                                <input className={css.ingredientNameInput}
                                autoFocus={true}
                                ...
                                {errors[`ingredients[${index}].name`] && (
                                    <p className={css.errorMsg}>{errors[`ingredients[${index}].name`]}</p>
                                )}
                        ...
                        <input className={css.ingredientQuantityInput}
                            type="number"
                            value={item.ingredientQuantity}
                            onChange={(e) => changeQuantity(index, e.target.value)}
                        />
                            <div className={css.selectBox} onClick={() => toggleQuantity(index)}>
                                <span className={css.selectLabel}>{ingredients[index].ingredientQuantity}</span>
                                <MdOutlineKeyboard className={css.arrow} size="18" />
                        </div>
                        {quantityIsActive[index] && (
                            <ul className={css.selectQuantityList}>
                            {ingredientsQuantityMeasure.map((item) => (
                                <li className={css.selectQuantityItem}
                                key={item}
                        ...
                        {errors[`ingredients[${index}].ingredientQuantityMeasure`] && (
                            <p className={css.errorMsg}> {errors[`ingredients[${index}].ingredientQuantityMeasure`]} </p>
                        )}
                        </div>
                        </div>
                        <button className={css.deleteBtn} type="button" onClick={() => deleteField(item.id, index)}>
